  ...
  async ({ formValue, navigate, toast }, { rejectWithValue }) => {
    try {
      const { data } = await axios.post(
        "https://blog-api1-z30x.onrender.com/api/users/signup",
        formValue
      );
...
export const login = createAsyncThunk(
  ...
  async ({ formValue, navigate, toast }, { rejectWithValue }) => {
    try {
      const { data } = await axios.post(
        "https://blog-api1-z30x.onrender.com/api/users/signin",
        formValue
      );
  ...
  async (paylaod, { rejectWithValue, getState }) => {
    ...
    try {
      const { data } = await axios.get(
        `https://blog-api1-z30x.onrender.com/api/users/singleUser/${user?.user?.id} `,
        config
      );
      return data;
    } catch (error) {
